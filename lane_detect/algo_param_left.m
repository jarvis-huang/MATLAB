function [P] = algo_param_left(img,rho_c,rho_r,theta_c,theta_r) % c-center, r-radius
...
i_cut(:,round(c_y/2):c_y,3) = 0; % remove right half
...
while (1)
    ...
    H(:,1:100) = 0; H(:,170:180) = 0; % slope has to be [10 80]
    ...
    % -- left marking --
    H_left(round(rho_c)+rho_r:ss(1),:) = 0; % restrict rho
    H_left(1:round(rho_c)-rho_r,:) = 0;

    P = houghpeaks(H_left,3,'threshold',ceil(0.05*max(H_left(:)))); % find 3 peaks
